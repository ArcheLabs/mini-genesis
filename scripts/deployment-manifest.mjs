...
export function validateManifest(manifest, environment, options = {}) {
  ...
  if (manifest.status === "deployed" || options.runtimeReady) { if (manifest.status !== "deployed") throw new Error(`TEMPLATE_MANIFEST_NOT_RUNTIME_READY_${environment}`); for (const value of [source.contract, source.genesisHash, source.runtimeCodeHash, source.deploymentBlock, destination.miniLucky, destination.trustGraph, destination.genesisHash, destination.deploymentBlock]) if (value === "0" || ZERO.test(value)) throw new Error(`ZERO_DEPLOYMENT_VALUE_${environment}`); const supported = options.supportedChainIds ?? []; if (!supported.includes(source.chainId) || !supported.includes(destination.chainId)) throw new Error(`UNSUPPORTED_CHAIN_ID_${environment}`); }
  return manifest;
}
