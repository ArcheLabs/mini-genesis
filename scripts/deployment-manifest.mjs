...
export function validateManifest(manifest, environment, options = {}) {
  ...
  check(destination.personhoodPrecompile, ADDRESS, "PERSONHOOD_PRECOMPILE");
...
function validateGenesisPhases(genesis, environment) {
  ...
  for (const [value, name] of [
    ...
    [phase2.endPriceX18, "GENESIS_PHASE2_END_PRICE"],
  ...
  for (const [value, name] of [
    [snapshot.allocationMini, "GENESIS_PHASE2_SNAPSHOT_ALLOCATION"],
    [snapshot.soldMini, "GENESIS_PHASE2_SNAPSHOT_SOLD"],
    [snapshot.raisedDot, "GENESIS_PHASE2_SNAPSHOT_RAISED"],
    [snapshot.buyerCount, "GENESIS_PHASE2_SNAPSHOT_BUYERS"],
    [snapshot.startPriceX18, "GENESIS_PHASE2_SNAPSHOT_START_PRICE"],
    [snapshot.terminalPriceX18, "GENESIS_PHASE2_SNAPSHOT_TERMINAL_PRICE"],
    [snapshot.startTime, "GENESIS_PHASE2_SNAPSHOT_START_TIME"],
    [snapshot.endTime, "GENESIS_PHASE2_SNAPSHOT_END_TIME"],
    [snapshot.deploymentBlock, "GENESIS_PHASE2_SNAPSHOT_DEPLOYMENT_BLOCK"],
  ]) check(required(value, name), DECIMAL, name);
  ...
  for (const [snapshotValue, phaseValue, name] of [
    [snapshot.allocationMini, phase2.allocationMini, "ALLOCATION"],
    [snapshot.startPriceX18, phase2.startPriceX18, "START_PRICE"],
    [snapshot.endPriceX18, phase2.endPriceX18, "END_PRICE"],
    [snapshot.startTime, phase2.startTime, "START_TIME"],
...
export async function readManifest(environment, options) {
  return validateManifest(JSON.parse(await readFile(resolve("deployments", `${environment}.json`), "utf8")), environment, options);
}
