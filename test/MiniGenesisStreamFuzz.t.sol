...
contract MiniGenesisStreamFuzzTest is Test {
    ...
    function testFuzzEmissionIsExactAndMonotonic(
        uint128 allocation,
        ...
        MiniGenesisStream stream = new MiniGenesisStream(
            treasury, allocation, contributionBlocks, protectionBlocks, 2, 1
        );
        vm.deal(alice, 2);
        vm.prank(alice);
        ...
        for (uint256 k; k <= 32; ++k) {
            vm.roll(stream.startBlock() + total * k / 32);
            uint256 current = stream.emittedMini();
