...
contract MiniGenesisStream is ReentrancyGuard {
    ...
    constructor(
        ...
        if (
            genesisAllocation_ == 0 ||
            contributionBlocks_ == 0 ||
            protectionBlocks_ == 0 ||
            firstContributionMinimum_ == 0 ||
    ...
    function contribute() external payable nonReentrant {
        bool isFirstContribution = startBlock == 0;

        if (isFirstContribution) {
            if (msg.value < firstContributionMinimum) {
                revert FirstContributionTooSmall();
            }
            ...
            emit GenesisStarted(
                startBlock,
                contributionEndBlock,
                emissionEndBlock,
                msg.sender,
                msg.value
            );
        } else {
            if (block.number >= contributionEndBlock) {
    ...
    function phase() external view returns (Phase) {
        ...
        if (block.number < emissionEndBlock) {
            return Phase.Protection;
        }
    ...
    function pendingMini(address account) public view returns (uint256) {
        ...
        uint256 accumulated = Math.mulDiv(
            user.contributedDot,
            _previewAccMiniPerDot(),
            ACC_PRECISION
        );

        uint256 unsettled = accumulated > user.rewardDebt
    ...
    function _previewAccMiniPerDot()
        internal
        view
        returns (uint256 currentAcc)
    {
        ...
        return
            currentAcc +
            Math.mulDiv(newEmission, ACC_PRECISION, totalRaisedDot);
    }
    ...
    function _cumulativeEmission(
        ...
        uint256 capped = elapsedBlocks > totalEmissionBlocks
            ? totalEmissionBlocks
            : elapsedBlocks;
