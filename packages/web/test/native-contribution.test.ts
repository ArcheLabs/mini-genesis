...
const signerMocks = vi.hoisted(() => ({ submit: vi.fn() }));
vi.mock("../src/wallet/substrate/injected-transaction", async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual, submitNativeReviveCall: signerMocks.submit };
});
...
function nativeApi() {
  ...
  signerMocks.submit.mockClear();
  signerMocks.submit.mockImplementation(async ({ onStatus }: { onStatus?: (status: string) => void }) => {
    onStatus?.("broadcast");
    onStatus?.("inBlock");
    onStatus?.("finalized");
    return {
  ...
  return { api, tx, call, submission: signerMocks.submit };
}
...
describe("native contribution adapter", () => {
  ...
  it("uses planck for Revive.call and validates the emitted Solidity event", async () => {
    const { api, submission } = nativeApi();
    ...
    expect(submission).toHaveBeenCalledWith(expect.objectContaining({ address: NATIVE_ACCOUNT, value: 10_000_000_000n }));
  ...
  it("does not sign when the dry-run reverts", async () => {
    const { api, submission, call } = nativeApi();
