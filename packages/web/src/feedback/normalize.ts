...
export function normalizeFeedback(error: unknown, context: FeedbackContext): NormalizedFeedback {
  ...
  const kind: FeedbackKind = code === "TRANSACTION_INCLUDED" ? "success" : code === "WALLET_CONNECTION_REJECTED" || code === "USER_REJECTED_TRANSACTION" ? "info" : code === "CONTRIBUTION_CLOSED" || code === "MAX_AMOUNT_UNAVAILABLE" || surface === "banner" ? "warning" : "error";
