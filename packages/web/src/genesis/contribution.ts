...
import { validateContributionAmount, type ParsedDotAmount } from "./amount";
...
export type ContributionResult = { hash: Hash; blockNumber: bigint; amount: ParsedDotAmount; execution: "evm" };
...
export async function contribute(client: PublicClient, wallet: WalletClient, manifest: DeploymentManifest, account: Address, input: string, phase: number, firstMinimum: bigint, subsequentExclusive: bigint, onUpdate: (update: ContributionUpdate) => void = () => {}, signal?: AbortSignal): Promise<ContributionResult> {
  try {
    ...
    const amount = validateContributionAmount(input, phase, firstMinimum, subsequentExclusive);
    const balance = await client.getBalance({ address: account }); checkCancelled(signal);
    if (balance < amount.evmWei) throw new Error("INSUFFICIENT_BALANCE");
    onUpdate({ state: "simulating" }); await client.simulateContract({ address: manifest.source.contract, abi: genesisAbi, functionName: "contribute", account, value: amount.evmWei } as any); checkCancelled(signal);
    onUpdate({ state: "awaiting_signature" }); const hash = await wallet.writeContract({ address: manifest.source.contract, abi: genesisAbi, functionName: "contribute", account, value: amount.evmWei } as any); checkCancelled(signal);
    ...
    for (const log of receipt.logs) {
      ...
      try { const decoded = decodeEventLog({ abi: genesisAbi, data: log.data, topics: log.topics }); if (decoded.eventName === "Contributed" && String((decoded.args as any).contributor).toLowerCase() === account.toLowerCase() && (decoded.args as any).amount === amount.evmWei) matches += 1; } catch { /* unrelated log */ }
    }
    if (matches !== 1) throw new Error("CONTRIBUTED_EVENT_MISMATCH");
    onUpdate({ state: "included", hash }); return { hash, blockNumber: receipt.blockNumber, amount, execution: "evm" };
