...
import { currentRuntimeSelection } from "../config/runtime-selection";

const demoMode = import.meta.env.VITE_DEMO_MODE === "true";
export const runtimeSelection = currentRuntimeSelection(import.meta.env.MODE, import.meta.env.VITE_DEPLOYMENT_ENV);
const manifest = getManifest(runtimeSelection.environment);
const projectId = resolveReownProjectId(import.meta.env);
const chainId = Number(manifest?.source.chainId ?? 0);
...
export const polkadotHubNetwork = defineChain({
  ...
  name: manifest?.source.name ?? "Configuration error",
