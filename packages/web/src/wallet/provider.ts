...
export async function switchChain(provider: Eip1193Provider, manifest: DeploymentManifest): Promise<void> {
  ...
  try { await provider.request({ method: "wallet_switchEthereumChain", params: [{ chainId }] }); }
    ...
    await provider.request({ method: "wallet_addEthereumChain", params: [{ chainId, chainName: manifest.source.name, nativeCurrency: { name: manifest.source.currencySymbol, symbol: manifest.source.currencySymbol, decimals: 18 }, rpcUrls: manifest.source.rpcHttpUrls.filter(Boolean), blockExplorerUrls: manifest.source.explorerUrl ? [manifest.source.explorerUrl] : [] }] });
