...
import { LuckyCreditClaim } from "./src/claim/LuckyCreditClaim";
...
function App() {
  ...
  const ecosystemAssetCard = <LuckyCreditClaim manifest={manifest} sourceH160={genesisIdentity} sessionKey={sessionKey} ss58Prefix={(session?.kind === "polkadot" ? nativeManifest : manifest)?.source.ss58Prefix ?? 0} provider={provider} canSign={Boolean(session?.kind === "evm" && provider && correctChain)} locale={language} onError={(error) => feedback.presentError(error, context("claim-credit"))} />;
