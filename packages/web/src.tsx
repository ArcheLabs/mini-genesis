...
import { useWalletSession } from "./src/wallet/use-genesis-wallet";
...
function App() {
  ...
  const { session, selectedPaymentSource, account, isConnected, status, correctChain, paymentReady, chainId, connectWallet, connectPolkadotExtension, availablePolkadotWallets, switchToGenesisChain, disconnect, selectPaymentSource, refreshPaymentSource, ensurePaymentSourceSigner } = useWalletSession(manifest, publicClient);
  const selectedContractAddress = selectedPaymentSource?.kind === "evm" ? selectedPaymentSource.address : selectedPaymentSource?.contractAddressStatus === "verified" ? selectedPaymentSource.contractIdentity : null;
  ...
  const selectedSourceIdRef = useRef(selectedPaymentSource?.id ?? null);
  selectedSourceIdRef.current = selectedPaymentSource?.id ?? null;
  ...
  useEffect(() => { const onPointerDown = (event: PointerEvent) => { if (walletMenu && walletWrapRef.current && !walletWrapRef.current.contains(event.target as Node)) setWalletMenu(false); if (balanceMenu && balanceWrapRef.current && !balanceWrapRef.current.contains(event.target as Node)) setBalanceMenu(false); if (languageMenu && languageWrapRef.current && !languageWrapRef.current.contains(event.target as Node)) setLanguageMenu(false); }; const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") { setWalletMenu(false); setBalanceMenu(false); setLanguageMenu(false); feedback.notifications.filter((item) => !item.persistent).forEach((item) => feedback.dismiss(item.dedupeKey)); } }; document.addEventListener("pointerdown", onPointerDown); document.addEventListener("keydown", onKeyDown); return () => { document.removeEventListener("pointerdown", onPointerDown); document.removeEventListener("keydown", onKeyDown); }; }, [balanceMenu, feedback.dismiss, feedback.notifications, languageMenu, walletMenu]);
  ...
  const calculateMax = useCallback(async (targetAccount: Address, nextDynamic: GenesisDynamic): Promise<bigint | null> => { const requestedSourceId = selectedPaymentSource?.id; if (demoMode || selectedPaymentSource?.kind !== "evm" || !publicClient || !manifest || !staticState || !requestedSourceId) return null; const value = await safeMaxAmount(publicClient, { account: targetAccount, contract: manifest.source.contract, phase: nextDynamic.phase, firstContributionMinimum: staticState.firstContributionMinimum, subsequentContributionMinimumExclusive: staticState.subsequentContributionMinimumExclusive }); if (selectedSourceIdRef.current !== requestedSourceId) return null; setMaxAmount(value); return value; }, [demoMode, manifest, publicClient, selectedPaymentSource, staticState]);
  const calculateNativeMax = useCallback(async (nextDynamic: GenesisDynamic): Promise<bigint | null> => { const requestedSourceId = selectedPaymentSource?.id; if (demoMode || selectedPaymentSource?.kind !== "polkadot" || !selectedPaymentSource.api || !manifest || !staticState || !requestedSourceId) { setNativeMaxAmount(null); return null; } const value = await estimateNativeMax(selectedPaymentSource.api, selectedPaymentSource.address, manifest, nextDynamic.phase, staticState.firstContributionMinimum, staticState.subsequentContributionMinimumExclusive); if (selectedSourceIdRef.current !== requestedSourceId) return null; setNativeMaxAmount(value); return value; }, [demoMode, manifest, selectedPaymentSource, staticState]);
  ...
  const loadUser = useCallback(async (targetAccount: Address, sourceId: string) => { if (demoMode || !publicClient || !manifest) return; try { const next = await readGenesisUserState(publicClient, manifest, targetAccount); if (selectedSourceIdRef.current !== sourceId) return; setUser(next); feedback.clearCode("USER_DATA_UNAVAILABLE"); } catch (error) { if (selectedSourceIdRef.current === sourceId) feedback.presentError(error, context("load-user")); } }, [context, demoMode, feedback.clearCode, feedback.presentError, manifest, publicClient]);
  const loadHistory = useCallback(async (targetAccount: Address, sourceId: string) => { if (demoMode || !publicClient || !manifest) return; try { const finalized = await publicClient.request({ method: "eth_getBlockByNumber", params: ["finalized", false] } as any) as { number?: string } | null; if (finalized?.number) { const next = await readContributionHistory(publicClient, manifest, targetAccount, BigInt(finalized.number)); if (selectedSourceIdRef.current !== sourceId) return; setHistory(next); } feedback.clearCode("HISTORY_UNAVAILABLE"); } catch (error) { if (selectedSourceIdRef.current === sourceId) feedback.presentError(error, context("load-history")); } }, [context, demoMode, feedback.clearCode, feedback.presentError, manifest, publicClient]);
  const clearUser = useCallback(() => { setUser(demoMode ? demoUser : null); setHistory([]); setMaxAmount(demoMode ? demoGenesis.walletBalance : null); feedback.clearCode("USER_DATA_UNAVAILABLE"); feedback.clearCode("HISTORY_UNAVAILABLE"); }, [feedback.clearCode]);
  ...
  useEffect(() => {
    ...
    if (selectedPaymentSource?.kind === "evm" && !correctChain) {
      maxRefreshKey.current = null;
      clearUser();
    ...
    feedback.clearCode("WRONG_CHAIN");
    feedback.clearCode("CHAIN_SWITCH_REJECTED");
  }, [clearUser, context, correctChain, feedback.clearCode, feedback.presentCode, manifest, paymentReady, selectedPaymentSource?.kind]);
  useEffect(() => {
    if (!paymentReady || !selectedContractAddress || demoMode) return;
    if (route === "assets") {
      void loadUser(selectedContractAddress, selectedPaymentSource?.id ?? "");
      void loadHistory(selectedContractAddress, selectedPaymentSource?.id ?? "");
      return;
    }
    void loadUser(selectedContractAddress, selectedPaymentSource?.id ?? "");
  }, [chainId, demoMode, loadHistory, loadUser, paymentReady, route, selectedContractAddress, selectedPaymentSource?.id]);
  useEffect(() => {
    if (demoMode) return;
    maxRefreshKey.current = null;
    setMaxAmount(null);
    setNativeMaxAmount(null);
  }, [demoMode, selectedPaymentSource?.id]);
  useEffect(() => {
    if (selectedPaymentSource?.kind !== "evm" || !account || !dynamicState || !staticState) return;
    const key = `${selectedPaymentSource.id}:${chainId}`;
    if (maxRefreshKey.current === key) return;
    maxRefreshKey.current = key;
    void calculateMax(account, dynamicState);
  }, [account, calculateMax, chainId, dynamicState, selectedPaymentSource, staticState]);
  useEffect(() => {
    if (selectedPaymentSource?.kind !== "polkadot" || !dynamicState) { setNativeMaxAmount(null); return; }
    setNativeMaxAmount(null);
    void calculateNativeMax(dynamicState);
  }, [calculateNativeMax, dynamicState, selectedPaymentSource]);
  ...
  const submit = async () => {
    if (contributionBusy || !selectedPaymentSource) return;
    ...
    if (!amount || !manifest || !dynamicState || !staticState || !publicClient) return;
    ...
    try {
      const source = selectedPaymentSource;
      ...
      if (source.kind === "polkadot") {
        if (!source.api) throw new Error("SUBSTRATE_RPC_UNAVAILABLE");
        adapter = createSubstrateExecutionAdapter(source.api, source.signer, source.address, manifest, source.contractAddressStatus === "verified" ? source.contractIdentity : undefined);
      } else {
        const readySource = await ensurePaymentSourceSigner(source);
        if (readySource.kind !== "evm" || !readySource.provider) throw new Error("EVM_SIGNER_UNAVAILABLE");
        if (!source.correctChain) await switchToGenesisChain();
        adapter = createEvmExecutionAdapter(publicClient, walletClient(readySource.provider, manifest), manifest, source.address);
      }
      ...
      if (selectedContractAddress) void loadUser(selectedContractAddress, source.id);
      void refreshPaymentSource(source.id);
      if (source.kind === "evm") { void Promise.allSettled([refreshDynamic(), calculateMax(source.address, dynamicState)]); void loadHistory(source.address, source.id); }
      else if (selectedContractAddress) void loadHistory(selectedContractAddress, source.id);
    } catch (error) {
      void refreshPaymentSource(selectedPaymentSource.id);
  ...
  const setAll = async () => { if (contributionBusy) return; try { const next = selectedPaymentSource?.kind === "polkadot" ? nativeMaxAmount ?? (dynamicState ? await calculateNativeMax(dynamicState) : null) : account ? maxAmount ?? (dynamicState ? await calculateMax(account, dynamicState) : null) : null; if (next === null) { feedback.presentCode("MAX_AMOUNT_UNAVAILABLE", context("calculate-max")); return; } if (next === 0n) { const normalized = feedback.presentCode("INSUFFICIENT_BALANCE", context("calculate-max")); setAmountFeedback(normalized); return; } setAmount(formatNative(next)); setAmountFeedback(null); } catch (error) { feedback.presentError(error, context("calculate-max")); } };
