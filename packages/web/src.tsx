...
const copy = {
  "zh-CN": {
    ...
    evmWallet: "EVM 钱包", polkadotWallet: "Polkadot 钱包", evmWalletDesc: "MetaMask、Rabby 等 EVM 钱包", polkadotWalletDesc: "SubWallet、Talisman 等波卡钱包", choosePolkadotWallet: "选择 Polkadot 钱包", noPolkadotWallet: "未检测到 Polkadot 钱包。", noSupportedAccount: "未找到兼容的 Polkadot 账户。", switchAccount: "切换账户", back: "返回",
  },
  en: {
    ...
    evmWallet: "EVM Wallet", polkadotWallet: "Polkadot Wallet", evmWalletDesc: "MetaMask, Rabby and EVM-compatible wallets", polkadotWalletDesc: "SubWallet, Talisman and Polkadot wallets", choosePolkadotWallet: "Choose Polkadot Wallet", noPolkadotWallet: "No Polkadot wallet detected.", noSupportedAccount: "No compatible Polkadot account found.", switchAccount: "Switch account", back: "Back",
  },
} as const;
...
function App() {
  ...
  sessionKeyRef.current = session?.kind === "evm" ? `evm:${session.address}` : session?.kind === "polkadot" ? `polkadot:${session.selectedAccountAddress}` : null;
  ...
  const loadUser = useCallback(async (targetAccount: Address, requestedKey: string) => { if (demoMode || !publicClient || !manifest) return; try { const next = await readGenesisUserState(publicClient, manifest, targetAccount); if (sessionKeyRef.current !== requestedKey) return; setUser(next); feedback.clearCode("USER_DATA_UNAVAILABLE"); } catch (error) { if (sessionKeyRef.current === requestedKey) feedback.presentError(error, context("load-user")); } }, [context, demoMode, feedback.clearCode, feedback.presentError, manifest, publicClient]);
  const loadHistory = useCallback(async (targetAccount: Address, requestedKey: string) => { if (demoMode || !publicClient || !manifest) return; try { const finalized = await publicClient.request({ method: "eth_getBlockByNumber", params: ["finalized", false] } as any) as { number?: string } | null; if (finalized?.number) { const next = await readContributionHistory(publicClient, manifest, targetAccount, BigInt(finalized.number)); if (sessionKeyRef.current !== requestedKey) return; setHistory(next); } feedback.clearCode("HISTORY_UNAVAILABLE"); } catch (error) { if (sessionKeyRef.current === requestedKey) feedback.presentError(error, context("load-history")); } }, [context, demoMode, feedback.clearCode, feedback.presentError, manifest, publicClient]);
  ...
  useEffect(() => {
    if (!session || !paymentReady || !selectedContractAddress || !sessionKey || demoMode) return;
    if (route === "assets") {
      void loadUser(selectedContractAddress, sessionKey);
      void loadHistory(selectedContractAddress, sessionKey);
      return;
    }
    void loadUser(selectedContractAddress, sessionKey);
  }, [demoMode, loadHistory, loadUser, paymentReady, route, selectedContractAddress, session, sessionKey]);
  useEffect(() => {
    if (demoMode) return;
    maxRefreshKey.current = null;
    setMaxAmount(null);
  ...
  useEffect(() => {
    if (session?.kind !== "polkadot" || !dynamicState) { setNativeMaxAmount(null); return; }
    setNativeMaxAmount(null);
    void calculateNativeMax(dynamicState);
  }, [calculateNativeMax, dynamicState, session, sessionKey]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (selectedContractAddress) void loadUser(selectedContractAddress, sessionKey);
      ...
      else if (selectedContractAddress) void loadHistory(selectedContractAddress, sessionKey);
    } catch (error) {
      if (session.kind === "polkadot") void refreshNativeBalance();
  ...
  const walletLabel = session ? shortHash(session.kind === "evm" ? session.address : session.selectedAccountAddress) : text.connect;
