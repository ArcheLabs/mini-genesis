...
function App() {
  ...
  useEffect(() => {
    ...
    if (route === "assets") {
      ...
      void loadHistory(genesisIdentity, sessionKey);
      return;
    }
    void loadUser(genesisIdentity, sessionKey);
  }, [demoMode, genesisIdentity, loadHistory, loadUser, paymentReady, route, sessionKey]);
  useEffect(() => {
    if (demoMode) return;
    maxRefreshKey.current = null;
    ...
    try {
      ...
      if (session.kind === "polkadot") {
        ...
        void reconcileGenesisUserState({ client: publicClient, manifest, identity: committedIdentity, expectedContributedDot: previousContributedDot + result.amount.evmWei, signal: controller.signal }).then((next) => {
          ...
          void loadHistory(committedIdentity, sessionKey);
  ...
  const walletLabel = session ? shortHash(session.kind === "evm" ? session.address : session.selectedAccountAddress) : walletStatus === "restoring" ? "Loading…" : text.connect;
