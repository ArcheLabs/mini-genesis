...
function App() {
  ...
  const genesisWallet = useGenesisWallet(manifest);
  const { account, provider, isConnected, status, correctChain, paymentReady, paymentKind, chainId, connect, connectPolkadot, openAccount, switchToGenesisChain, disconnect, disconnectPolkadot, substrateAccounts, selectPolkadotAccount, substrateAccount, substrateSigner, substrateApi, substrateContractAddress, mappingState, nativeBalance, refreshNativeBalance } = genesisWallet;
  const selectedContractAddress = substrateContractAddress ?? account;
  ...
  const [walletBalance, setWalletBalance] = useState<bigint | null>(demoMode ? demoGenesis.walletBalance : null);
  ...
  const calculateMax = useCallback(async (targetAccount: Address, nextDynamic: GenesisDynamic): Promise<bigint | null> => { if (demoMode || paymentKind === "substrate" || !publicClient || !manifest || !staticState) return null; const value = await safeMaxAmount(publicClient, { account: targetAccount, contract: manifest.source.contract, phase: nextDynamic.phase, firstContributionMinimum: staticState.firstContributionMinimum, subsequentContributionMinimumExclusive: staticState.subsequentContributionMinimumExclusive }); setMaxAmount(value); return value; }, [manifest, paymentKind, publicClient, staticState]);
  const calculateNativeMax = useCallback(async (nextDynamic: GenesisDynamic): Promise<bigint | null> => { if (demoMode || paymentKind !== "substrate" || !substrateApi || !substrateAccount || !manifest || !staticState || mappingState !== "mapped") { setNativeMaxAmount(null); return null; } const value = await estimateNativeMax(substrateApi, substrateAccount.address, manifest, nextDynamic.phase, staticState.firstContributionMinimum, staticState.subsequentContributionMinimumExclusive); setNativeMaxAmount(value); return value; }, [demoMode, manifest, mappingState, paymentKind, staticState, substrateAccount, substrateApi]);
  ...
  const loadUser = useCallback(async (targetAccount: Address) => { if (demoMode || !publicClient || !manifest) return; try { setUser(await readGenesisUserState(publicClient, manifest, targetAccount)); feedback.clearCode("USER_DATA_UNAVAILABLE"); } catch (error) { feedback.presentError(error, context("load-user")); } }, [context, feedback.clearCode, feedback.presentError, manifest, publicClient]);
  const loadBalance = useCallback(async (targetAccount: Address) => { if (demoMode || !publicClient || paymentKind !== "evm") return; try { setWalletBalance(await publicClient.getBalance({ address: targetAccount })); } catch (error) { feedback.presentError(error, context("load-user")); } }, [context, feedback.presentError, paymentKind, publicClient]);
  ...
  useEffect(() => {
    ...
    if (paymentKind === "evm" && !correctChain) {
      maxRefreshKey.current = null;
      clearUser();
    ...
    feedback.clearCode("WRONG_CHAIN");
    feedback.clearCode("CHAIN_SWITCH_REJECTED");
  }, [clearUser, context, correctChain, feedback.clearCode, feedback.presentCode, manifest, paymentKind, paymentReady]);
  useEffect(() => {
    if (!paymentReady || !selectedContractAddress || demoMode) return;
    if (route === "assets") {
      void loadUser(selectedContractAddress);
      if (paymentKind === "evm" && account) void loadHistory(account);
      return;
    }
    void loadUser(selectedContractAddress);
    if (paymentKind === "evm" && account) void loadBalance(account);
  }, [account, chainId, demoMode, loadBalance, loadHistory, loadUser, paymentKind, paymentReady, route, selectedContractAddress]);
  useEffect(() => {
    if (!paymentReady || paymentKind !== "substrate" || !substrateContractAddress || demoMode || route !== "assets") return;
    void loadHistory(substrateContractAddress);
  }, [demoMode, loadHistory, paymentKind, paymentReady, route, substrateContractAddress]);
  useEffect(() => {
    if (paymentKind !== "evm" || !account || !dynamicState || !staticState) return;
  ...
  }, [account, calculateMax, chainId, dynamicState, paymentKind, staticState]);
  useEffect(() => {
    if (paymentKind !== "substrate" || !dynamicState || mappingState !== "mapped") { setNativeMaxAmount(null); return; }
    void calculateNativeMax(dynamicState);
  }, [calculateNativeMax, dynamicState, mappingState, paymentKind, nativeBalance]);
  ...
  const submit = async () => { if (contributionBusy || !selectedContractAddress && !demoMode) return; setAmountFeedback(null); feedback.clearOperation("submit-contribution"); if (demoMode) { setContributionState("demo_processing"); window.setTimeout(() => { setContributionState("idle"); feedback.presentCode("TRANSACTION_INCLUDED", context("submit-contribution")); }, 700); return; } if (!amount || !manifest || !dynamicState || !staticState || !selectedContractAddress) return; const minimum = dynamicState.phase === 0 ? staticState.firstContributionMinimum : staticState.subsequentContributionMinimumExclusive; try { const contextValue = { manifest, phase: dynamicState.phase, firstMinimum: staticState.firstContributionMinimum, subsequentExclusive: staticState.subsequentContributionMinimumExclusive, contractAddress: selectedContractAddress }; const adapter = paymentKind === "substrate" && substrateApi && substrateAccount && substrateSigner ? createSubstrateExecutionAdapter(substrateApi, substrateSigner, substrateAccount.address, manifest, substrateContractAddress ?? undefined) : account && publicClient && provider ? createEvmExecutionAdapter(publicClient, walletClient(provider, manifest), manifest, account) : null; if (!adapter) return; const result = await adapter.contribute(amount, contextValue, (update) => setContributionState(update.state)); setAmount(""); setContributionState("idle"); const txHash = result.evmTransactionHash ?? result.substrateTransactionHash; feedback.presentCode("TRANSACTION_INCLUDED", context("submit-contribution", { transactionHash: txHash, explorerUrl: manifest.source.explorerUrl })); if (selectedContractAddress) void loadUser(selectedContractAddress); if (paymentKind === "substrate") void refreshNativeBalance(); if (account && paymentKind === "evm") { void Promise.allSettled([refreshDynamic(), loadBalance(account), calculateMax(account, dynamicState)]); void loadHistory(account); } } catch (error) { if (paymentKind === "substrate") void refreshNativeBalance(); setContributionState("idle"); const normalized = feedback.presentError(error, context("submit-contribution", { minimum: formatNative(minimum), explorerUrl: manifest.source.explorerUrl })); if (normalized.surface === "field") setAmountFeedback(normalized); } };
  const setAll = async () => { if (contributionBusy) return; try { const next = paymentKind === "substrate" ? nativeMaxAmount ?? (dynamicState ? await calculateNativeMax(dynamicState) : null) : account ? maxAmount ?? (dynamicState ? await calculateMax(account, dynamicState) : null) : null; if (next === null) { feedback.presentCode("MAX_AMOUNT_UNAVAILABLE", context("calculate-max")); return; } if (next === 0n) { const normalized = feedback.presentCode("INSUFFICIENT_BALANCE", context("calculate-max")); setAmountFeedback(normalized); return; } setAmount(formatNative(next)); setAmountFeedback(null); } catch (error) { feedback.presentError(error, context("calculate-max")); } };
  ...
  const walletLabel = substrateAccount ? shortHash(substrateAccount.address) : account ? shortHash(account) : text.connect;
  ...
  const header = <header className="site-header"><nav className="nav"><a className="brand" href="#/" onClick={(event) => { event.preventDefault(); navigate("genesis"); }} aria-label="MINI Home"><span className="brand-mark">M</span><span className="brand-word">MINI</span></a><div className="nav-center"><a className={`nav-link ${route === "genesis" ? "active" : ""}`} href="#/" onClick={(event) => { event.preventDefault(); navigate("genesis"); }}>{icons.rocket}{text.genesis}</a><a className={`nav-link ${route === "rules" ? "active" : ""}`} href="#/rules" onClick={(event) => { event.preventDefault(); navigate("rules"); }}>{icons.book}{text.rules}</a></div><div className="nav-actions"><div className="language-wrap" ref={languageWrapRef}><button className="language-button" type="button" aria-label={text.language} aria-haspopup="listbox" aria-expanded={languageMenu} onClick={() => setLanguageMenu((value) => !value)}>{icons.globe}{icons.chevron}</button>{languageMenu && <div className="wallet-menu language-menu open" role="listbox" aria-label={text.language}>{([["zh-CN", "中文"], ["en", "EN"]] as const).map(([value, label]) => <button key={value} type="button" role="option" aria-selected={language === value} className={language === value ? "selected" : ""} onClick={() => { setLanguage(value); setLanguageMenu(false); }}>{icons.globe}{label}{language === value && <span className="check">✓</span>}</button>)}</div>}</div><button className="utility-button" type="button" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} aria-label="Switch appearance"><span className="utility-icon">{theme === "dark" ? "☀" : "☾"}</span></button><div className="wallet-wrap" ref={walletWrapRef}><button className="wallet-button" type="button" onClick={() => !paymentReady ? setWalletMenu((value) => !value) : paymentKind === "evm" && !correctChain ? void switchToGenesisChain().catch((error) => feedback.presentError(error, context("switch-network", { networkName: manifest?.source.name }))) : setWalletMenu((value) => !value)}><span className="wallet-dot" hidden={!paymentReady} />{icons.wallet}<span className="wallet-label">{walletLabel}</span>{paymentReady && icons.chevron}</button>{walletMenu && <div className="wallet-menu open">{!paymentReady && <><button type="button" onClick={() => void connectPolkadot().catch((error) => feedback.presentError(error, context("connect-wallet")))}>Polkadot Wallet</button><button type="button" onClick={() => void connect()}>{text.connect} · Ethereum</button></>}{paymentReady && <><button type="button" onClick={() => navigate("assets")}>{icons.wallet}{text.myAssets}</button>{paymentKind === "substrate" && substrateAccounts.length > 1 && substrateAccounts.map((candidate) => <button key={candidate.address} type="button" className={candidate.address === substrateAccount?.address ? "selected" : ""} onClick={() => selectPolkadotAccount(candidate.address)}>{candidate.name || shortHash(candidate.address)}{candidate.address === substrateAccount?.address && <span className="check">✓</span>}</button>)}{paymentKind === "evm" && <AccountSwitchMenuItem label={walletAccountMenuText[language]} onClick={() => void openAccount()} />}{paymentKind === "evm" && <button type="button" className="danger" onClick={disconnect}>{icons.disconnect}{text.disconnect}</button>}{paymentKind === "substrate" && <button type="button" className="danger" onClick={disconnectPolkadot}>{icons.disconnect}{text.disconnect}</button>}</>}</div>}</div></div></nav></header>;
